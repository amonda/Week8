...
head(ophelia_lines)
#w/o value=true it does not tell you what the lines say just where they are 
```
...
#grep tells you ham appears 358 times, grepl tells you 0 or 1 for each variable its there or not 
```
...
hamlet_lines <- grep("  Ham\\.", hamlet, value = TRUE) #\\ treats . as a . instead of a metacharacter 
length(hamlet_lines)
...
head(grep("H[b-z]", hamlet, value = TRUE)) #says I want H and anything besides a
```
...
head(grep("  H(a|o)", hamlet, value = TRUE)) #find anything starting with Ha or Ho
...
head(grep("^  H[b-z]", hamlet, value = TRUE)) #^anchors to beginning so you find where he speaks not just where he is mentioned $anchors to the end
...
sum(Hamlet) #gives you number of times they actually speak
sum(Ophelia)
sum(Polonius)
sum(Gertrude)
sum(Laertes)
